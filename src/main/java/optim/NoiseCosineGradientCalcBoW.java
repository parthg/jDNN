...
public class NoiseCosineGradientCalcBoW extends GradientCalc {

  public NoiseCosineGradientCalcBoW(Batch _batch) {
    super(_batch);
  }
...
//    System.out.printf("calling getValue()\n");
    ...
    if(System.getProperty("use_cuda").equals("true")) {
      ...
      DMatrix ARep = this.model.fProp(_batch.data());
      DMatrix BRep = this.model.fProp(_batch.pos());
      DMatrix NRep = this.model.fProp(_batch.neg());
    ...
    // make it parallel
      ...
      throw new UnsupportedOperationException("UNIMPLEMENTED: BLAS version of NoiseCosineGradientCalcBoW.");
...
//    System.out.printf("calling getValue()\n");
    ...
    if(System.getProperty("use_cuda").equals("true")) {
      ...
      this.model.copyHtoD();
      DMatrix ARep = this.model.fProp(this.batch.data());
      DMatrix BRep = this.model.fProp(this.batch.pos());
      DMatrix NRep = this.model.fProp(this.batch.neg());
...
//        DMatrix b3 = b.pow(3.0);
        ...
        // df/dA = (A, B) - (A, N)
        DMatrix tempGrad = this.model.bProp(this.batch.data(), ARep, dAB);
        grads.addi(tempGrad);
        tempGrad.close();

        tempGrad = this.model.bProp(this.batch.data(), ARep, dAN);
        grads.subi(tempGrad);
        tempGrad.close();

        // df/dB = (B, A)
        tempGrad = this.model.bProp(this.batch.pos(), BRep, dBA);
        grads.addi(tempGrad);
        tempGrad.close();

        // df/dN = - (N, A)
        tempGrad = this.model.bProp(this.batch.neg(), NRep, dNA);
        grads.subi(tempGrad);
        tempGrad.close();
