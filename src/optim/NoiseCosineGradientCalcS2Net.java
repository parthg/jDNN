...
public class NoiseCosineGradientCalcS2Net extends GradientCalc {

  public NoiseCosineGradientCalcS2Net(Batch _batch) {
    super(_batch);
  }
...
//    System.out.printf("calling getValue()\n");
    ...
    if(System.getProperty("use_cuda").equals("true")) {
      ...
      DMatrix ABoW = this.prepareSumMatrix(_batch.data(), _batch.dataWordsArray());
      DMatrix BBoW = this.prepareSumMatrix(_batch.pos(), _batch.posWordsArray());
      DMatrix NBoW = this.prepareSumMatrix(_batch.neg(), _batch.negWordsArray());

      DMatrix ARep = this.model.fProp(ABoW);
      DMatrix BRep = this.model.fProp(BBoW);
      DMatrix NRep = this.model.fProp(NBoW);
...
/*        A.copyHtoD();
        ...
        DMatrix ABoW = this.prepareSumMatrix(this.batch.data(), this.batch.dataWordsArray());
        DMatrix BBoW = this.prepareSumMatrix(this.batch.pos(), this.batch.posWordsArray());
        DMatrix NBoW = this.prepareSumMatrix(this.batch.neg(), this.batch.negWordsArray());
...
//        DMatrix b3 = b.pow(3.0);
        ...
        // df/dA = (A, B) - (A, N)
        DMatrix tempGrad = this.model.bProp(ABoW, ARep, dAB);
        grads.addi(tempGrad);
        tempGrad.close();

        tempGrad = this.model.bProp(ABoW, ARep, dAN);
        grads.subi(tempGrad);
        tempGrad.close();

        // df/dB = (B, A)
        tempGrad = this.model.bProp(BBoW, BRep, dBA);
        grads.addi(tempGrad);
        tempGrad.close();

        // df/dN = - (N, A)
        tempGrad = this.model.bProp(NBoW, NRep, dNA);
        grads.subi(tempGrad);
        tempGrad.close();
